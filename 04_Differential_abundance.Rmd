...
sample_metadata$region <- gsub("Ittoqqortoormii", "Ittoqqortoormiit", sample_metadata$region) # correct region names if they are not already
```
...
#Generate phyloseq object with structural zeros
...
no_ittoq %>%
	filter(species == "s__") %>% dplyr::select(c(1:6))
cat(new_species_ittoq*100/total_absent_ittoq)
```
...
LinDA_table_phylum <- rownames_to_column(LinDA_phylum$output$regionIttoqqortoormiit, "genomes") 
