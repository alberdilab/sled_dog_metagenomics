...
table_upset_analysis <- genome_counts_row[, colnames(genome_counts_row) %in% daneb_samples]%>%
...
***Total number only present in Ittoqqortoormii and absent in Daneborg***
```{r speno_total_ittoq, comment="", echo=FALSE, message=FALSE, warning=FALSE}
no_daneb <- table_upset_analysis_binary %>% dplyr::select(Daneb) %>% dplyr::filter(.,Daneb==0)%>%rownames_to_column(., "genome")%>%
      dplyr::left_join(., genome_metadata, by = join_by(genome == genome)) %>% dplyr::select(c(1,4:9))
total_absent <- no_daneb %>%
	nrow()
total_absent
```

***Mags only present in Ittoqqortoormii with species-level annotation***
```{r spe_ittoq, comment="", echo=FALSE, message=FALSE, warning=FALSE}
no_daneb%>%
	filter(species != "s__")
```

***Number of MAGs present only in Ittoqqortoormii without species-level annotation***
```{r speno_perc_ittoq, comment="", echo=FALSE, message=FALSE, warning=FALSE}
new_species_absent <- no_daneb %>%
	filter(species == "s__") %>%
	nrow()
no_daneb %>%
...
***Total number only present in Daneborg and absent in Ittoqqortoormii***
```{r speno_total_daneb, comment="", echo=FALSE, message=FALSE, warning=FALSE}
no_ittoq <- table_upset_analysis_binary %>% dplyr::select(Ittoq) %>% dplyr::filter(.,Ittoq==0)%>%rownames_to_column(., "genome")%>%
      dplyr::left_join(., genome_metadata, by = join_by(genome == genome)) %>% dplyr::select(c(1,4:9))
total_absent_ittoq <- no_ittoq %>%
	nrow()
total_absent_ittoq
```
...
new_species_ittoq <- no_ittoq %>%
	filter(species == "s__") %>%
	nrow()
no_ittoq %>%
	filter(species == "s__") %>% dplyr::select(c(1:6))
new_species_ittoq*100/total_absent_ittoq
```
...
physeq_phylum <- microbiome::aggregate_taxa(genome_data, 'phylum')
physeq_phylum_rel <-  microbiome::transform(physeq_phylum, "compositional")
physeq_phylum_rel_itto <- subset_samples(physeq_phylum_rel, region == "Ittoqqortoormii")
...
for(i in colNames){
  plt <- ggplot(significant_phylum_table, aes(x=region, y=.data[[i]], color = region)) +
...
significant_family <- clr_t_family %>%
    pivot_longer(-c(sample,region), names_to = "Family", values_to = "value") %>%
    group_by(Family) %>%
    summarise(p_value = wilcox.test(value ~ region, exact = FALSE, alternative = "less")$p.value) %>%
    mutate(p_adjust=p.adjust(p_value, method="BH")) %>%
    filter(p_adjust < 0.05)
...
physeq_family <- microbiome::aggregate_taxa(genome_data, 'family')
...
for(i in colNames){
  plt <- ggplot(significant_family_table, aes(x=region, y=.data[[i]], color = region)) +
    geom_boxplot(alpha = 0.2, outlier.shape = NA, width = 0.3, show.legend = FALSE) +
  geom_jitter(width = 0.1, show.legend = TRUE) +
  theme_minimal() +
  theme(
...
physeq_genus <- microbiome::aggregate_taxa(genome_data, 'genus')
...
colNames <- names(significant_phylum_table)[1:2]
for(i in colNames){
  plt <- ggplot(significant_phylum_table, aes(x=region, y=.data[[i]], color = region)) +
    geom_boxplot(alpha = 0.2, outlier.shape = NA, width = 0.3, show.legend = FALSE) +
  geom_jitter(width = 0.1, show.legend = TRUE) +
...
colNames <- names(significant_family_table)[1:3]
for(i in colNames){
  plt <- ggplot(significant_family_table, aes(x=region, y=.data[[i]], color = region)) +
    geom_boxplot(alpha = 0.2, outlier.shape = NA, width = 0.3, show.legend = FALSE) +
  geom_jitter(width = 0.1, show.legend = TRUE) +
...
diagdds.ins.fis <- results(diagdds, alpha=0.01, contrast=c("region", "Ittoqqortoormii", "Daneborg"))
...
diagdds = phyloseq_to_deseq2(physeq_genome_filtered, ~ region)
...
genome_counts_filtered.clr <- aldex.clr(genome_counts_aldex, 
               sample_metadata$region, #not a factor
               mc.samples=128, 
               denom="all", 
               verbose=F)
```
...
LinDA_table_phylum <- rownames_to_column(LinDA_phylum$output$regionIttoqqortoormii, "asv") 
