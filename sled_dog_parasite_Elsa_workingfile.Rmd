...
ehi_phylum_colors <- read.table("/Users/elsa/Desktop/THESIS/sled_dog_metagenomics/resources/ehi_phylum_colors.tsv",sep="\t",header=T,comment.char = "")
...
#    facet_nested(.~Individual+Sample_type,  scales="free_x") +
   ...
   labs(fill="Phylum")
```
# 5 Alpha diversity calculations
...
colNames <- names(element_gift_names)[2:60] # change to change number of plotted elements
for(i in colNames){
...
colNames <- names(function_gift_names)[2:7] # change to change number of plotted functions
for(i in colNames){
...
# DA
## all run after removing structural zeros
...
structural_zeros <- genome_counts %>% 
   ...
   mutate(all_zeros_Ittoq = all(c_across(all_of(Ittoqqortoormiit_samples)) == 0)) %>% # set true if all samples in Ittoq have zeros
   mutate(all_zeros_Daneborg = all(c_across(all_of(Daneborg_samples)) == 0)) %>% # set true if all samples in Daneborg have zeros
   mutate(average_Ittoq = mean(c_across(all_of(Ittoqqortoormiit_samples)), na.rm = TRUE)) %>% # get average genome counts across Ittoq
...
```{r spe_ittoq, comment="", echo=FALSE, message=FALSE, warning=FALSE}
no_daneborg%>%
...
```{r speno_perc_ittoq, comment="", echo=FALSE, message=FALSE, warning=FALSE}
...
total_absent_Ittoqqortoormiit <- no_Ittoqqortoormiit %>%
	nrow()
total_absent_Ittoqqortoormiit # 37 MAGs only in Daneborg
```

***Mags only present in Daneborg with species-level annotation***
```{r spe_daneb, comment="", echo=FALSE, message=FALSE, warning=FALSE}
no_Ittoqqortoormiit%>%
...
```{r speno_perc_daneb, comment="", echo=FALSE, message=FALSE, warning=FALSE}
new_species_Ittoqqortoormiit <- no_Ittoqqortoormiit %>%
	filter(species == "s__") %>%
	nrow()
no_Ittoqqortoormiit %>%
	filter(species == "s__") %>% dplyr::select(c(1:6))
new_species_Ittoqqortoormiit*100/total_absent_Ittoqqortoormiit # 21 Daneborg unique MAGs w/o species-level annotation
```
### Phylum percentages
...
colNames <- names(significant_genome_table)[1:10] # change to change number of plotted MAGs
for(i in colNames){
...
colNames <- names(significant_genus_table)[1:5] # change to change number of plotted MAGs
for(i in colNames){
...
ggplot(ancombc_rand_table, aes(x=forcats::fct_rev(taxon), y=lfc_regionIttoqqortoormii, color=phylum)) + 
  geom_point(size=4) + 
...
ancom_result_genus %>%
    mutate(significance = ifelse(q_regionIttoqqortoormiit < 0.05, "1", "0")) %>%
...
```{r}
ancom_result_genus %>%
    filter(q_regionIttoqqortoormiit < 0.05) %>%
    ggplot(., aes(x=lfc_regionIttoqqortoormiit, y=forcats::fct_rev(genus), color=phylum)) +
      geom_jitter(size=3) +
      geom_vline(xintercept=0) + 
      xlim(-max(ancom_result_genus$lfc_regionIttoqqortoormiit)-2,max(ancom_result_genus$lfc_regionIttoqqortoormiit)+2) +
      scale_color_manual(values=colors_alphabetic$colors) +
      geom_text(aes(-max(ancom_result_genus$lfc_regionIttoqqortoormiit)+1, 1), label = "Enriched\nin Daneborg", color="#666666") +
      geom_text(aes(max(ancom_result_genus$lfc_regionIttoqqortoormiit)-1, 1), label = "Enriched\nin Ittoq", color="#666666") +
     theme(legend.position='none',
          panel.background = element_blank(),
          axis.line = element_line(size = 0.5, linetype = "solid", colour = "black"),
...
ancom_result_genus %>%
  ...
  filter(q_regionIttoqqortoormiit < 0.05) %>% # note use of q here
  arrange(lfc_regionIttoqqortoormiit) %>%
  dplyr::select(genome,lfc_regionIttoqqortoormiit,q_regionIttoqqortoormiit) %>%
  kable()
...
ancom_result_genus %>%
  filter(q_regionIttoqqortoormiit < 0.05) %>% 
  ggplot(., aes(x=forcats::fct_rev(genome), y=lfc_regionIttoqqortoormiit, color=phylum)) + 
  geom_point(size=4) + 
  scale_color_manual(values=colors_alphabetic$colors) + 
  geom_hline(yintercept=0) + 
  coord_flip()+
...
LinDA_table <- rownames_to_column(LinDA$output$regionIttoqqortoormiit, "genome") 
```
...
LinDA_table_genus <- rownames_to_column(LinDA_genus$output$regionIttoqqortoormiit, "genus") 
```
```{r LinDA DA G summary, comment="", echo=FALSE, message=FALSE, warning=FALSE}
tax <- data.frame(physeq.genus_LinDA@tax_table) %>%
  rownames_to_column(., "taxon")

LinDA_table_genus <- rownames_to_column(LinDA_genus$output$regionIttoqqortoormiit, "genome") 
...
ANCOMBC_genome <- ancom_result %>%
  filter(q_regionIttoqqortoormiit < 0.05) %>%
  dplyr::select(., genome, q_regionIttoqqortoormiit, lfc_regionIttoqqortoormiit) %>%
  dplyr::rename(
    padj_ANCOMBC = q_regionIttoqqortoormiit,
...
selected_columns_func <- sample_metadata %>%
...
count_table_rda_binary <- ifelse(genome_counts > 0, 1, 0) %>%
  as.data.frame()
count_table_rda_binary_ittoq <- count_table_rda_binary[, colnames(count_table_rda_binary) %in% Ittoqqortoormiit_samples]
...
Ittoqqortoormiit_summed <- count_table_rda_binary_ittoq[, c('sum_of_rows', "genome")]
colnames(Ittoqqortoormiit_summed)[colnames(Ittoqqortoormiit_summed) == 'sum_of_rows'] <- 'Ittoq'
...
upsetrda_1 <- merge(Ittoqqortoormiit_summed, daneborg_summed, by = "genome", all =T )
...
# OTU table
MAGotu_zeros <- genome_counts %>% # genome size normalized counts
  column_to_rownames("genome") %>%
  otu_table(., taxa_are_rows = TRUE)
# Tax table
...
summary(sample_metadata$Eukaryota_.)
# Range 3-13 % reads mapped to Eukaryota
# Med = 8.5%, Mean = 8.517

# Shapiro-Wilk test for normality
shapiro_results_Euk <- sample_metadata %>%
  group_by(region) %>%
...
wilcox_exact_results_Euk <- exactRankTests::wilcox.exact(Eukaryota_. ~ region, data = sample_metadata)
print(wilcox_exact_results_Euk) # Results: p = 0.6329 # THEREFORE no sig difference in % Eukaryotic reads between regions

# Plot
ggplot(sample_metadata, aes(x = region, y = Eukaryota_., group = region, color = region)) +
  geom_boxplot(alpha = 0.2, outlier.shape = NA, width = 0.3, show.legend = FALSE) +
  geom_jitter(width = 0.1, show.legend = TRUE) +
  theme_minimal() +
  theme(
...
# Wilcoxon Exact rank test
library(exactRankTests)
wilcox_exact_results_Crypto_R <- exactRankTests::wilcox.exact(Cryptosporidium_. ~ region, data = sample_metadata)
...
wilcox_results_Crypto_PR <- sample_metadata %>%
  filter(PR_cryptosporidium != "excluded") %>%
  group_by(PR_cryptosporidium) %>%
...
# Plot by region
ggplot(sample_metadata, aes(x = region, y = Cryptosporidium_., group = region, color = region)) +
  geom_boxplot(alpha = 0.2, outlier.shape = NA, width = 0.3, show.legend = FALSE) +
  geom_jitter(width = 0.1, show.legend = TRUE) +
  theme_minimal() +
  theme(
...
# Plot by PR results
ggplot(sample_metadata %>% filter(PR_cryptosporidium != "excluded"),
...
# Plot by region
ggplot(sample_metadata %>% filter(Giardia_. != 0), aes(x = region, y = Giardia_., group = region, color = region)) +
  geom_boxplot(alpha = 0.2, outlier.shape = NA, width = 0.3, show.legend = FALSE) +
  geom_jitter(width = 0.1, show.legend = TRUE) +
  theme_minimal() +
  theme(
...
# Plot by PR results
ggplot(sample_metadata %>% filter(PR_giardia != "excluded") %>% filter(Giardia_. != 0),
...
summary(sample_metadata$Sarcocystis_.)
# Range 0 - 3.000e-04 
...
summary(sample_metadata$Toxoplasma_gondii_.)
# Range 0.001 - 0.03 ; Med = 0.004, Mean = 0.004897 
means_T_gondii_region <- sample_metadata %>%
  group_by(region) %>%
  summarize(mean_T_gondii = mean(Toxoplasma_gondii_., na.rm = TRUE)) # Daneborg 0.00445 ; Ittoq 0.00534

# Shapiro-Wilk test for normality (grouped by region)
shapiro_results_Toxoplasma_gondii_R <- sample_metadata %>%
  group_by(region) %>%
...
# Wilcoxon Exact rank test
wilcox_exact_results_Toxoplasma_gondii_R <- exactRankTests::wilcox.exact(Toxoplasma_gondii_. ~ region, data = sample_metadata)
...
# Plot by region
ggplot(sample_metadata, aes(x = region, y = Toxoplasma_gondii_., group = region, color = region)) +
  geom_boxplot(alpha = 0.2, outlier.shape = NA, width = 0.3, show.legend = FALSE) +
  geom_jitter(width = 0.1, show.legend = TRUE) +
  theme_minimal() +
  theme(
...
# Wilcoxon Exact rank test
wilcox_exact_results_Trichinella_R <- exactRankTests::wilcox.exact(Trichinella_. ~ region, data = sample_metadata)
print(wilcox_exact_results_Trichinella_R) # Results: p = 0.0002889 significant difference in Trichinella between regions

# Plot by region
ggplot(sample_metadata, aes(x = region, y = Trichinella_., group = region, color = region)) +
  geom_boxplot(alpha = 0.2, outlier.shape = NA, width = 0.3, show.legend = FALSE) +
  geom_jitter(width = 0.1, show.legend = TRUE) +
  theme_minimal() +
  theme(
...
summary(sample_metadata$Taenia_.)
# Range 0 - 0.008
# Med = 0, Mean = 0.0004343
mean(subset(sample_metadata, Taenia_. != 0)$Taenia_.) #0.001145
means_Taenia_region <- sample_metadata %>% 
  filter(Taenia_. != 0) %>%
  group_by(region) %>%
  summarize(mean_Taenia = mean(Taenia_., na.rm = TRUE)) # Daneborg 0.00101 ; Ittoq 0.0017

# Shapiro-Wilk test for normality (grouped by region)
shapiro_results_Taenia_R <- sample_metadata %>%
  group_by(region) %>%
  summarise(shapiro_p_value = shapiro.test(Taenia_.)$p.value) #Results: p = 1.03e- 9 Daneborg, p = 1.45e-11 Ittoq

# Wilcoxon Exact rank test
wilcox_exact_results_Taenia_R <- sample_metadata %>%
...
# Plot by region
ggplot(sample_metadata %>% filter(Taenia_. != 0), aes(x = region, y = Taenia_., group = region, color = region)) +
  geom_boxplot(alpha = 0.2, outlier.shape = NA, width = 0.3, show.legend = FALSE) +
  geom_jitter(width = 0.1, show.legend = TRUE) +
  theme_minimal() +
  theme(
...
summary(sample_metadata$Uncinaria_.)
# Range 0 - 0.02 
# Med = 0, Mean = 0.0003843
summary((subset(sample_metadata, Uncinaria_. != 0)$Uncinaria_.))
means_Uncinaria_region <- sample_metadata %>% 
  filter(Uncinaria_. != 0) %>%
  group_by(region) %>%
  summarize(mean_Uncinaria = mean(Uncinaria_., na.rm = TRUE)) # Daneborg 0.0012 ; Ittoq 0.000072

# Shapiro-Wilk test for normality
shapiro_results_Uncinaria <- sample_metadata %>%
  group_by(region) %>%
  summarise(shapiro_p_value = shapiro.test(Uncinaria_.)$p.value) # Results: p = 1.71e-11 Daneborg, p =  3.81e-10 Ittoq

# Wilcoxon Exact rank test
wilcox_exact_results_Uncinaria_R <- sample_metadata %>%
...
# Plot
ggplot(sample_metadata, aes(x = region, y = Uncinaria_., group = region, color = region)) +
  geom_boxplot(alpha = 0.2, outlier.shape = NA, width = 0.3, show.legend = FALSE) +
  geom_jitter(width = 0.1, show.legend = TRUE) +
  theme_minimal() +
  theme(
...
summary(sample_metadata$Echinococcus_.) # Range 0.003 - 0.08  Med = 0.00850, Mean = 0.01167
means_Echinococcus_region <- sample_metadata %>% 
  filter(Echinococcus_. != 0) %>%
  group_by(region) %>%
  summarize(mean_Echinococcus = mean(Echinococcus_., na.rm = TRUE)) # Daneborg 0.013 ; Ittoq 0.010

# Shapiro-Wilk test for normality
shapiro_results_Echinococcus <- sample_metadata %>%
  group_by(region) %>%
  summarise(shapiro_p_value = shapiro.test(Echinococcus_.)$p.value) # Results: p = 0.00000000897 Daneborg, p =  0.00000784 Ittoq

# Wilcoxon Exact rank test
wilcox_exact_results_Echinococcus <- exactRankTests::wilcox.exact(Echinococcus_. ~ region, data = sample_metadata)
print(wilcox_exact_results_Echinococcus) # Results: p =0.312 THEREFORE no sig difference between regions

# Plot
ggplot(sample_metadata, aes(x = region, y = Echinococcus_., group = region, color = region)) +
  geom_boxplot(alpha = 0.2, outlier.shape = NA, width = 0.3, show.legend = FALSE) +
  geom_jitter(width = 0.1, show.legend = TRUE) +
  theme_minimal() +
  theme(
...
summary(sample_metadata$Cestoda_.) # Range 0.06 - 0.9  Med = 0.1, Mean = 0.1129

# Shapiro-Wilk test for normality
shapiro_results_Cestoda <- sample_metadata %>%
  group_by(region) %>%
  summarise(shapiro_p_value = shapiro.test(Cestoda_.)$p.value) # Results: p = 3.39e-11 Daneborg, p =  1.03e- 9 Ittoq

# Wilcoxon Exact rank test
wilcox_exact_results_Cestoda <- exactRankTests::wilcox.exact(Cestoda_. ~ region, data = sample_metadata)
...
summary(sample_metadata$Nematoda_.) # Range   0.02 - 0.4  Med = 0.3, Mean = 0.259

# Shapiro-Wilk test for normality
shapiro_results_Nematoda <- sample_metadata %>%
  group_by(region) %>%
  summarise(shapiro_p_value = shapiro.test(Nematoda_.)$p.value) # Results: p = 0.00000112 Daneborg, p =   0.00000262 Ittoq

# Wilcoxon Exact rank test
wilcox_exact_results_Nematoda <- exactRankTests::wilcox.exact(Nematoda_. ~ region, data = sample_metadata)
print(wilcox_exact_results_Nematoda) # Results: p =0.3481 no sig difference between regions
```
# Viruses
Viral overall
```{r Virus}
summary(sample_metadata$Virus_.) # Range 0.3-4.0 % reads mapped to Virus Med = 0.07%, Mean = 0.225

# Shapiro-Wilk test for normality
shapiro_results_Virus <- sample_metadata %>%
  group_by(region) %>%
  summarise(shapiro_p_value = shapiro.test(Virus_.)$p.value) # Results: p = 0.000237 Daneborg, p = 0.00000000206 Ittoq

# Wilcoxon Exact rank test
library(exactRankTests)
wilcox_exact_results_Virus <- exactRankTests::wilcox.exact(Virus_. ~ region, data = sample_metadata)
print(wilcox_exact_results_Virus) # Results: p = 0.002321 YES sig diff in % Virus reads between regions

# Plot
ggplot(sample_metadata, aes(x = region, y = Virus_., group = region, color = region)) +
  geom_boxplot(alpha = 0.2, outlier.shape = NA, width = 0.3, show.legend = FALSE) +
  geom_jitter(width = 0.1, show.legend = TRUE) +
  theme_minimal() +
  theme(
...
# Parvo
library(exactRankTests)
summary(sample_metadata$PARVO_Parvoviridae_.)
wilcox_exact_results_Parvo <- exactRankTests::wilcox.exact(sample_metadata$PARVO_Parvoviridae_. ~ region, data = sample_metadata)
print(wilcox_exact_results_Parvo) # Results: p = 7.547e-09 sig diff in % Viral reads mapped to Parvo between regions
means_Parvoviridae_region <- sample_metadata %>% 
...
# Adenovirus
library(exactRankTests)
wilcox_exact_results_Adeno <- exactRankTests::wilcox.exact(sample_metadata$ADENO_Adenoviridae_. ~ region, data = sample_metadata)
print(wilcox_exact_results_Adeno) # Results: p = 0.0001022  sig diff in % Viral reads mapped to Adeno between regions
means_Adeno_region <- sample_metadata %>% 
...
#Papilloma
library(exactRankTests)
wilcox_exact_results_Pap <- exactRankTests::wilcox.exact(sample_metadata$PAP_Papillomaviridae_. ~ region, data = sample_metadata)
