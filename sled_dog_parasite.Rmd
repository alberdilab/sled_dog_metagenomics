...
###################### Elsa's check of structural zeros ###################### 
count_table_cov_size_pivot <- t(count_table_cov_size) %>%
  as.data.frame() 
count_table_cov_size_pivot_sample <- rownames_to_column(count_table_cov_size_pivot, var = "sample")
count_table_cov_size_pivot_meta <- merge(count_table_cov_size_pivot_sample, all_metadata[, c("sample", "region")], by.x = "sample", by.y = "sample", all.x = TRUE)
count_table_cov_size_pivot_meta <- column_to_rownames(count_table_cov_size_pivot_meta, "sample")
ittoq_rows <- count_table_cov_size_pivot_meta[count_table_cov_size_pivot_meta$region == "Ittoqqortoormii", ]
daneborg_rows <- count_table_cov_size_pivot_meta[count_table_cov_size_pivot_meta$region == "Daneborg", ]

column_sums_ittoq <- colSums(ittoq_rows[, sapply(ittoq_rows, is.numeric) & !(names(ittoq_rows) %in% c("sample", "region"))])
zero_sum_columns_ittoq <- names(column_sums_ittoq[column_sums_ittoq == 0])
column_sums_Daneborg <- colSums(daneborg_rows[, sapply(daneborg_rows, is.numeric) & !(names(daneborg_rows) %in% c("sample", "region"))])
zero_sum_columns_Daneborg <- names(column_sums_Daneborg[column_sums_Daneborg == 0])
###################### Results match ###################### 
...
```
there are 246 MAGs enriched in ittoq compared to Daneborg, 138 showing decreased expression in ittoq compared to daneborg
...
# only difference is the addition of the psudocounts
...
ancom_result_genus %>%
  ...
  arrange(lfc_regionIttoqqortoormii) %>%
  dplyr::select(genus,lfc_regionIttoqqortoormii,phylum,q_regionIttoqqortoormii) %>%
  kable()
...
tse_LinDA = mia::makeTreeSummarizedExperimentFromPhyloseq(physeq) # create tse object
...
ggplot(LinDA_table, aes(x=forcats::fct_rev(genome), y=log2FoldChange, color=phylum)) + 
  geom_point(size=4) + 
  scale_color_manual(values=colors_alphabetic$colors) + 
  geom_hline(yintercept=0) + 
  coord_flip()+
...
count_table_rda_binary <- ifelse(count_table_rda > 0, 1, 0) %>%
  as.data.frame()
...
table_upset_analysis <- as.data.frame(table_upset_analysis)
rows_with_zero <- rownames(table_upset_analysis)[which(table_upset_analysis$Daneborg == 0 | table_upset_analysis$Ittoqqortoormii == 0)]
```
##RDA
