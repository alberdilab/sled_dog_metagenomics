...
structural_zeros %>%
    ...
    ggplot(., aes(x=average, y=forcats::fct_rev(phylum), color=phylum)) +
     ...
     theme(legend.position='none',
      ...
      guides(col=guide_legend("Phylum"))
```
```{r new count table without structural zeros}
...
# OTU table
MAGotu <- otu_table(count_table_cov_size_DA, taxa_are_rows = T) # normalized counts, structural zeros removed
# Tax table
taxonomyclean1 <- taxonomyclean[,-1]
taxmatrix <- as.matrix(taxonomyclean1[1:8])
rownames(taxmatrix) <- taxonomyclean$genome
...
regions <- c("Daneborg", "Ittoqqortoormii")

summary_table.W_filtered_meta <- table.W_filtered_meta %>%
  group_by(region) %>%
  summarise(
    mean_Actinomycetota = mean(Actinomycetota),
    sd_Actinomycetota = sd(Actinomycetota),
    mean_Bacillota_A = mean(Bacillota_A),
    sd_Bacillota_A = sd(Bacillota_A)
  ) %>%
...
p2<- table.W_filtered_meta %>%
  ggplot(aes(x = region, y = Actinomycetota, group = region, color = region)) +
  geom_boxplot(alpha = 0.2, outlier.shape = NA, width = 0.3, show.legend = FALSE, coef = 0) +
  geom_jitter(width = 0.1, show.legend = TRUE) +
# geom_text(label = "p-adj= 0.0003") +
  theme(panel.background = element_blank(),
...
summary_table.W_filtered_meta <- table.W_filtered_meta %>%
  group_by(region) %>%
  summarise(
    mean_Collinsella = mean(Collinsella),
    sd_Collinsella = sd(Collinsella),
    mean_Blautia_A = mean(Blautia_A),
    sd_Blautia_A = sd(Blautia_A),
    mean_Oliverpabstia = mean(Oliverpabstia),
    sd_Oliverpabstia = sd(Oliverpabstia),
    mean_Allobaculum = mean(Allobaculum),
    sd_Allobaculum = sd(Allobaculum),
    mean_Paraprevotella = mean(Paraprevotella),
    sd_Paraprevotella = sd(Paraprevotella),
    mean_Bacteria_Deferribacterota_Deferribacteres_Deferribacterales_Mucispirillaceae_ = mean(Bacteria_Deferribacterota_Deferribacteres_Deferribacterales_Mucispirillaceae_),
    sd_Bacteria_Deferribacterota_Deferribacteres_Deferribacterales_Mucispirillaceae_ = sd(Bacteria_Deferribacterota_Deferribacteres_Deferribacterales_Mucispirillaceae_),
  ) %>%
  filter(region %in% regions)
```
...
## DESeq2
```{r DESEq2 Phyloseq object}
phylo_samples_deseq <- all_metadata %>% 
                    column_to_rownames("sample") %>% 
                    mutate(region = factor(region, levels = c("Daneborg","Ittoqqortoormii"))) %>% #TJ1 = Daneborg, TJ2 = Ittoq
                    sample_data() #convert to phyloseq sample_data object

phylo_counts_deseq <- counts_table %>% # DESeq performs its own normalization, so the raw count table is added
...
phylo_taxonomy_deseq <- taxonomyclean %>% 
...
physeq_deseq <- phyloseq(phylo_counts_deseq, phylo_taxonomy_deseq, phylo_samples_deseq)
```
### Phylum level
```{r DESeq2 DA P, comment="", echo=FALSE, message=FALSE, warning=FALSE}
# aggregate at phylum level
physeq.phylum <- tax_glom(physeq_deseq, 'phylum')
...
physeq.genus <- tax_glom(physeq_deseq, 'genus')
...
ancom_result_phylum %>%
  ...
  filter(p_regionIttoqqortoormii < 0.05) %>% # note use of p here, should we switch to q?
