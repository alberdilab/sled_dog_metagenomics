...
```{r kegg_tree, comment="", echo=FALSE, message=FALSE, warning=FALSE}
kegg_tree <- force.ultrametric(tree,method="extend") %>%
                ggtree(., size = 0.3)
```
```{r kegg_tree_plot, comment="", echo=FALSE, message=FALSE, warning=FALSE}
...
#Add KEGG heatmap
kegg_tree <- gheatmap(kegg_tree, genome_gifts, offset=0.5, width=3.5, colnames=FALSE) +
            vexpand(.08) +
...
```{r tse, comment="", echo=FALSE, message=FALSE, warning=FALSE}
